...
def _img_from_cam(cam, pts):
    # Newer pycolmap requires 3D camera-space points; cam_from_img returns 2D
    # points on the z=1 normalized plane, so pad with a homogeneous 1.
    pts = np.asarray(pts)
    if pts.shape[-1] == 2:
        pts = np.concatenate([pts, np.ones((*pts.shape[:-1], 1), dtype=pts.dtype)], axis=-1)
    return np.asarray(cam.img_from_cam(pts))
...
def compute_mapping(H: int, W: int, fx: float, fy: float, cx: float, cy: float, k1: float, k2: float, p1: float, p2: float, k3: float, blank_pixels: bool, device: str = 'cpu'):
    ...
    undistorted_points1 = _img_from_cam(undistorted_camera, points1_in_cam)
    points2_in_cam = camera.cam_from_img(ys2)
    undistorted_points2 = _img_from_cam(undistorted_camera, points2_in_cam)
